...
/*
 ...
 do-catch:
 Used when you want to properly handle errors and respond differently depending on the type of error.
 It allows the program to catch specific errors and display appropriate messages or take corrective actions.

 
 
 try?:
 Used when you do not need detailed information about the error.
 It converts the result into an optional value and returns nil if an error occurs, which makes the code simpler when error details are not important.

 
 
 try!:
 Used when you are absolutely sure that the function will not throw an error.
 If an error does occur, the program will crash. Therefore, it should only be used with values that are guaranteed to be valid.
 */
...
if let result = findFirst(in: names, where: { $0.hasPrefix("C") }) {
    print(result)
}
...
print(intStack.peek() ?? "Empty")   // 30
print(intStack.pop() ?? "Empty")    // 30
